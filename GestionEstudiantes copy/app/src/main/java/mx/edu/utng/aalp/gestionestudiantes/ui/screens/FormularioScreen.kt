package mx.edu.utng.aalp.gestionestudiantes.ui.screens
...
import mx.edu.utng.aalp.gestionestudiantes.data.model.Estudiante
...
// Botón de guardar
            Button(
                onClick = {
                    // 1. Conversión y Validación Inicial
                    val edadInt = edad.toIntOrNull()
                    val promedioDouble = promedio.toDoubleOrNull()

                    // 2. Definición de la Condición de Validación (Booleano)
                    ...
                    if (esValido) {
                        // 3. Lógica de Éxito: Si todos los campos son válidos
                        onGuardarClick(nombre, edadInt!!, carrera, promedioDouble!!)
                    } else {
                        // 4. Lógica de Error: Si hay campos inválidos, activar mensajes de error
...
// Parámetros del Composable del Botón
                modifier = Modifier.fillMaxWidth(),
                enabled = !isLoading
            ) { // Cierre de los parámetros del Composable del Botón
                // Contenido del Botón (Text o CircularProgressIndicator)
                if (isLoading) {
                    ...
                    Text(text = if (esEdicion) "Actualizar" else "Guardar")
                }
            }
        }
    }
